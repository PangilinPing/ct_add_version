...
# PDF 輸出路徑
pdf_output_path = os.path.join(base_dir, 'all_plots.pdf')

# ================= 只保留四種資料來源 =================
candidates = [
    ('ct_rf_results.csv',            'CT + RF',          '#1f77b4'),
    ('rf_comparison_results.csv',     'RF',               '#2ca02c'),
    ('mlp_incremental_results.csv',   'DNN',              '#d62728'),
...
TIME_CONFIGS = [
    ('CT + RF', 'ct_rf_results.csv', '#1f77b4',
     lambda row: _val(row, 'ct_stat_time_s') + _val(row, 'rf_fit_time_s')),

    ('RF', 'rf_comparison_results.csv', '#2ca02c',
     lambda row: _val(row, 'fit_time_s')),

    ('mlp', 'mlp_comparison_results.csv', '#d62728',
     lambda row: _val(row, 'fit_time_s')),
...
for r in ranges:
    ...
    plt.tight_layout()
    save_and_track(fig, os.path.join(output_dir, f"AUROC_Comparison_{r['name']}.png"))
    ...
    if has_data:
        plt.legend(fontsize=10)
    plt.tight_layout()
    save_and_track(fig, os.path.join(output_dir, f"Accuracy_Comparison_{r['name']}.png"))
    ...
    if has_data:
        plt.legend(fontsize=10)
    plt.tight_layout()
    save_and_track(fig, os.path.join(output_dir, f"Balanced_Accuracy_Comparison_{r['name']}.png"))
...
if series_time:
    ...
    for r in ranges:
        ...
        plt.tight_layout()
        save_and_track(fig, os.path.join(output_dir, f"Time_vs_Samples_{r['name']}_10roundavg.png"))
